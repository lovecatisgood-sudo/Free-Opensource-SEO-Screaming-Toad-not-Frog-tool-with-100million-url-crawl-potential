...
WHERE cu.crawl_id=? AND l.link_kind='internal' AND (' '||lower(replace(l.rel,',',' '))||' ') LIKE '% nofollow %' GROUP BY p.id`, []any{crawlID, now, crawlID}},
		...
		if err := refreshIssueProvenance(ctx, tx, crawlID); err != nil {
			return err
		}
		return nil
	})
...
func refreshIssueProvenance(ctx context.Context, tx *sql.Tx, crawlID contracts.ID) error {
	_, err := tx.ExecContext(ctx, `UPDATE issue SET
classification = CASE
