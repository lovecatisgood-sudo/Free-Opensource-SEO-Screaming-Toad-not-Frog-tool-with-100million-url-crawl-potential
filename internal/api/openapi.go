...
var openAPIDocument = map[string]any{
	...
	"paths": map[string]any{
		...
		"/secrets":                                        map[string]any{"put": operation("Store an integration credential in the operating-system secret store"), "delete": operation("Delete an integration credential reference")},
		"/secrets/status":                                 map[string]any{"get": operation("Check whether a secure operating-system credential store is available")},
		...
		"/projects/{projectId}/schedules":                 map[string]any{"get": operation("List scheduled audits"), "post": operation("Create a bounded recurring audit")},
		"/projects/{projectId}/schedules/{scheduleId}":    map[string]any{"delete": operation("Delete a scheduled audit")},
		"/projects/{projectId}/integrations/{provider}":   map[string]any{"post": operation("Run an explicitly selected external integration")},
		"/projects/{projectId}/integrations/observations": map[string]any{"get": operation("List persisted external integration observations")},
		...
		"/crawls/{crawlId}/architecture":     map[string]any{"get": operation("Read a bounded internal-link architecture graph with segment and orphan metrics")},
