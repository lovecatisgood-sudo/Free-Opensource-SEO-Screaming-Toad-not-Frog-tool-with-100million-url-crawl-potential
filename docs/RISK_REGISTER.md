...
| SQLite write contention interrupts local campaigns | high | storage maintainer | single in-process writer, WAL, bounded batches, migration-free read-only diagnostics, durable recovery | 1M synthetic campaign recovered and passed; repeat/profiling still required at 5–10M |
| Disk exhaustion corrupts or strands a campaign | high | storage maintainer | actual usage ceiling, projected final usage, safe terminal reason, checksummed segments, backups | 1M campaign recovered after managed relocation; validate reserve-based pause before supported multi-million release |
...
| Capacity claim overstates verified behaviour | high | release maintainer | qualifying-field contract, campaign verifier, explicit synthetic/live transport labels, public-language gate | 1M synthetic production-path evidence passed; 100M+ claim prohibited until Stage D live guarded campaign passes |
