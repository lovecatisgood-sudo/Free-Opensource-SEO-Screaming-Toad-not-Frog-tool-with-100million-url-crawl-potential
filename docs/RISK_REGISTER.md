...
| SQLite write contention interrupts local campaigns | high | storage maintainer | single in-process writer, WAL, bounded batches, migration-free read-only diagnostics, durable recovery | controlled scale run recovered; profile at 1M and 10M gates |
| Disk exhaustion corrupts or strands a campaign | high | storage maintainer | actual usage ceiling, projected final usage, safe terminal reason, checksummed segments, backups | validate reserve-based pause before supported multi-million release |
...
| Capacity claim overstates verified behaviour | high | release maintainer | qualifying-field contract, campaign verifier, explicit synthetic/live transport labels, public-language gate | 100M+ claim prohibited until Stage D live guarded campaign passes |
