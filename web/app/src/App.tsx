...
import { auditSummary, bootstrap, cancelCrawl, createExport, crawlStatus, issues, pages, pauseCrawl, resumeCrawl, startCrawl, type CrawlProgress, type IssueRecord, type PageRecord, type Summary } from "./api";
...
export function App() {
  ...
  async function pause() { if (!crawl) return; try { await pauseCrawl(crawl.crawl_id); setCrawl(await crawlStatus(crawl.crawl_id)); } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not pause crawl"); } }
  async function resume() { if (!crawl) return; try { await resumeCrawl(crawl.crawl_id); setCrawl(await crawlStatus(crawl.crawl_id)); } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not resume crawl"); } }
  async function exportWorkbook(){if(!crawl)return;try{const artifact=await createExport(crawl.crawl_id,"workbook","xlsx");setArtifactMessage(`Workbook ready: ${artifact.path}`);}catch(reason){setError(reason instanceof Error?reason.message:"Could not create report");}}
  ...
  return <main className="shell">
    ...
    {crawl && <section className="progress-panel" aria-labelledby="crawl-progress-heading"><div><p className="kicker">Campaign status</p><h2 id="crawl-progress-heading">{crawl.status.replaceAll("_", " ")}</h2><p className="mono">{crawl.crawl_id}</p></div><div className="progress-content"><progress max={progressMaximum} value={progressValue}>{progressValue} of {progressMaximum}</progress><div className="metrics"><Metric label="Discovered" value={crawl.discovered}/><Metric label="Queued" value={crawl.queued}/><Metric label="Fetched" value={crawl.fetched}/><Metric label="Analysed" value={crawl.analysed}/><Metric label="Failed" value={crawl.failed}/></div><div className="actions">{crawl.status === "running" && <button className="secondary" onClick={pause}>Pause</button>}{crawl.status === "paused" && <button className="secondary" onClick={resume}>Resume</button>}{!terminal.has(crawl.status) && <button className="secondary danger" onClick={cancel}>Cancel crawl</button>}</div></div></section>}
    {summary && <section className="results" aria-labelledby="results-heading"><div className="section-heading"><p className="kicker">Audit evidence</p><h2 id="results-heading">Results</h2><p>Findings are technical observations with stored rule versions and evidence—not guarantees of indexing or rankings.</p></div><div className="summary-grid"><Metric label="Errors" value={summary.issues_by_severity.error ?? 0}/><Metric label="Warnings" value={summary.issues_by_severity.warning ?? 0}/><Metric label="Information" value={summary.issues_by_severity.info ?? 0}/><Metric label="Analysed pages" value={summary.analysed}/></div><div className="actions"><button className="secondary" onClick={exportWorkbook}>Export XLSX workbook</button><span aria-live="polite">{artifactMessage}</span></div><div className="tabs" role="tablist" aria-label="Audit result type"><button role="tab" aria-selected={view === "issues"} onClick={() => setView("issues")}>Issues</button><button role="tab" aria-selected={view === "pages"} onClick={() => setView("pages")}>Pages</button></div>{view === "issues" ? <IssueTable rows={issueRows}/> : <PageTable rows={pageRows}/>}</section>}
  </main>;
}
...
function IssueTable({rows}:{rows:IssueRecord[]}) { return <div className="table-wrap"><table><thead><tr><th>Severity</th><th>Rule</th><th>Evidence</th></tr></thead><tbody>{rows.map((row)=><tr key={row.id}><td><span className={`severity ${row.severity}`}>{row.severity}</span></td><td className="mono">{row.rule_id}</td><td><code>{row.evidence_json}</code></td></tr>)}</tbody></table>{rows.length===0&&<p className="empty">No issues in this result page.</p>}</div>; }
function PageTable({rows}:{rows:PageRecord[]}) { return <div className="table-wrap"><table><thead><tr><th>Status</th><th>URL</th><th>Title</th><th>Depth</th></tr></thead><tbody>{rows.map((row)=><tr key={row.id}><td>{row.status_code}</td><td><span className="url-cell">{row.url}</span></td><td>{row.title || <span className="muted">Missing</span>}</td><td>{row.depth}</td></tr>)}</tbody></table>{rows.length===0&&<p className="empty">No analysed pages.</p>}</div>; }
